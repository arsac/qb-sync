...
| `qbsync_qb_rechecks_total` | `reason` | Auto-triggered qB rechecks during finalization (destination) - `post_add_error` (a freshly-added torrent landed in an error state, typically NFS attribute-cache staleness on destination qB's mount), `parked` (destination qB already held the torrent in a download-side stopped state it would never leave on its own - usually the data was already on disk and something else added the torrent) |
