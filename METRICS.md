...
| `qbsync_orphan_cleanup_skipped_total` | `reason` | Orphan-cleanup attempts suppressed by safety checks — `in_qb` (torrent registered in destination qB but not healable: download-side/checking/error state, <100%, or its savepath is not qb-sync's copy — never deleted, never marked), `qb_unreachable` (destination qB returned an error during the safety check; sustained increments mean orphans accumulating because qB is offline / misconfigured) (destination) |
| `qbsync_orphan_cleanup_healed_total` | | Orphans self-healed to finalized: stale unfinalized metadata whose torrent destination qB reports complete on the seeding side AND at qb-sync's own savepath — qB verified qb-sync's data, so the `.finalized` marker is truthful. Typically the crash window between AddTorrent and the marker write. Savepath mismatches (operator/cross-seed copies of the same hash) keep the skip so `.meta` survives for eventual cleanup (destination) |
