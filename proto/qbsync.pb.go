...
const (
	PieceErrorCode_PIECE_ERROR_NONE            PieceErrorCode = 0 // Default / unknown (backward-compatible with old destination)
	...
	PieceErrorCode_PIECE_ERROR_NOT_INITIALIZED PieceErrorCode = 3 // Torrent not tracked on destination
...
func (FinalizeErrorCode) Descriptor() protoreflect.EnumDescriptor {
	return file_proto_qbsync_proto_enumTypes[1].Descriptor()
}

func (FinalizeErrorCode) Type() protoreflect.EnumType {
	return &file_proto_qbsync_proto_enumTypes[1]
}
...
// TorrentSyncStatus indicates the current state of a torrent on destination.
...
const (
	...
	// COMPLETE: Torrent already exists in destination qBittorrent and is verified.
	// No action needed - source can delete this torrent if space is needed.
	...
	// Source should wait and retry InitTorrent after a delay.
...
func (TorrentSyncStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_proto_qbsync_proto_enumTypes[2].Descriptor()
}

func (TorrentSyncStatus) Type() protoreflect.EnumType {
	return &file_proto_qbsync_proto_enumTypes[2]
}
...
type InitTorrentResponse struct {
	...
	// Status indicates what action source should take
	...
	// Count of pieces that need streaming (derived from pieces_needed).
	// Special value -1 means torrent not yet initialized on destination
	// (i.e., a minimal CheckTorrentStatus call — caller should send full InitTorrent).
	// Must stay in sync with pieces_needed when status is SYNC_STATUS_READY.
...
// Used for internal signaling/queueing between source components.
type Piece struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TorrentHash   string                 `protobuf:"bytes,1,opt,name=torrent_hash,json=torrentHash,proto3" json:"torrent_hash,omitempty"`
...
var file_proto_qbsync_proto_enumTypes = make([]protoimpl.EnumInfo, 3)
var file_proto_qbsync_proto_msgTypes = make([]protoimpl.MessageInfo, 19)
