...
func TestSetupFile_PreExisting(t *testing.T) {
	...
	t.Run("detects pre-existing file with correct size", func(t *testing.T) {
		...
		if fileInfo.hardlink.state != hlStateComplete {
			t.Errorf("expected hlStateComplete, got %v", fileInfo.hardlink.state)
		}
...
func TestInitTorrent_RelocatesOnSubPathChange(t *testing.T) {
	...
	// initAndRestart calls InitTorrent, then removes in-memory state to simulate a destination restart.
	...
	t.Run("relocates files from empty to category sub-path", func(t *testing.T) {
		...
		// Init with empty sub-path, then simulate destination restart
	...
	t.Run("no relocation when sub-path unchanged", func(t *testing.T) {
		...
		// Init with sub-path "movies", then simulate destination restart
...
func TestInitTorrentResync_ExistingState(t *testing.T) {
	...
	// selected, and source sends resync=true. Destination has in-memory state with file2
