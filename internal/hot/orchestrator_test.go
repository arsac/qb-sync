...
func TestDeleteGroupFromHot(t *testing.T) {
	...
	t.Run("happy path: stop → start cold → delete", func(t *testing.T) {
		...
		_ = task.deleteGroupFromHot(context.Background(), group)
	...
	t.Run("min seeding time not met skips group", func(t *testing.T) {
		mockClient := &mockQBClient{}
		coldDest := &mockColdDest{}
		task := &QBTask{
			cfg: &config.HotConfig{
				MinSeedingTime: 7 * 24 * time.Hour, // 7 days
			},
			logger:    logger,
			srcClient: mockClient,
			grpcDest:  coldDest,
		...
		group := torrentGroup{
			torrents: []qbittorrent.Torrent{
				{Hash: "abc123", SeedingTime: 100}, // 100 seconds, way below 7 days
			},
			minSeeding: 100,
		}
		_ = task.deleteGroupFromHot(context.Background(), group)
		...
		if mockClient.stopCalled {
			t.Error("StopCtx should NOT have been called when seeding time not met")
		}
		if coldDest.startCalled {
			t.Error("StartTorrent should NOT have been called when seeding time not met")
		}
	})
