...
const (
	...
	ReasonRelayFailed    Reason = "relay_failed"     // destination could not be asked - SYNC fail-open
)
...
type Decision struct {
	Sync   bool
	Reason Reason
	// Instance that decided, empty when the category routes to no instance.
	// Carried on the decision so a caller can label metrics without repeating
	// the routing lookup, which after the relay split lives only on the
	// destination anyway.
	Instance string
}

// CheckItem identifies one torrent for a batch verdict lookup.
